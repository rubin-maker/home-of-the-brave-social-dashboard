...
- The weekly category charts include a **Subscribers / Follows** mode. Instagram uses the source `Follows` value and YouTube uses the source `Subscribers` value, aggregated by each post or video's publish week and reviewed category. TikTok is explicitly unavailable, not zero. These current source-snapshot values are not account balances or a record of when followers or subscribers changed. The signed chart scale preserves negative YouTube subscriber adjustments.
