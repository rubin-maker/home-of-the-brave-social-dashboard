...
const sparkSummary=(platform,week)=>{const row=sparkWeekMap[platform][week],unit=D.definitions[platform].metric_label;return {value:`${fmt(row.views)} <span class="value-unit">${esc(unit)}</span>`,detail:`${chartDate(row.start)}–${chartDate(row.end)} · ${fmt(row.eng)} engagements · ${row.posts} posts · ${row.er??"—"}% rate`}};
const sparkDetail=(platform,week)=>{const row=sparkWeekMap[platform][week],unit=D.definitions[platform].metric_label,coverage=row.complete?(row.key===defaultSparkWeek[platform]?"Latest complete platform week":"Complete publish week"):`Partial publish week through ${chartDate(row.end)} · ${coveredDays(row.start,row.end)} of 7 days`;return `<strong>Selected week · ${esc(chartDate(row.start))}–${esc(chartDate(row.end))}, ${row.start.slice(0,4)}</strong><span>${full(row.views)} ${unit} · ${full(row.eng)} engagements · ${full(row.posts)} posts · ${row.er??"—"}% rate · ${coverage}</span>`};
...
document.getElementById("platforms").innerHTML=PLATS.slice().sort((a,b)=>RS.totals[b].views-RS.totals[a].views).map(p=>{
 ...
 <div class="spark-help">Latest 8 available publish weeks. The latest complete week is selected; select any bar for exact stats. The outlined final bar is partial.</div>${spark(p,PC[p])}<div class="spark-readout" aria-live="polite">${sparkDetail(p,sparkSelection[p])}</div><div class="detail">YTD: ${fmt(D.totals[p].views)} ${unit} across ${full(D.totals[p].posts)} posts</div></div>`}).join("");
