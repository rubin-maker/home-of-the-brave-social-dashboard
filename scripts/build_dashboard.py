...
data = {
    ...
    "completeThrough": source["platform_complete_through"],
    "latestPost": source["platform_latest_post_date"],
...
<div class="sub">A unified view of YouTube, Instagram, TikTok, and X. Top summary numbers use the shared latest complete Monday–Sunday week; each platform card opens on that source's latest complete week, and the explorer retains the full year-to-date file.</div></div>
...
<div class="note" style="margin-top:7px">This table keeps the eight complete comparison weeks through __SCOPE__. The mini bars above use each platform's latest eight available publish weeks and outline any partial source-coverage weeks. X uses impressions; the other platforms use views.</div></section>
...
const postCount=n=>`${full(n)} ${n===1?"post":"posts"}`;
const rate=(e,v)=>v?(100*e/v).toFixed(2)+"%":"—";
const delta=(a,b)=>!a?"—":`${b>=a?"+":""}${Math.round(100*(b-a)/a)}%`;
const reportWeekKey=D.scope.start;
const platformSparkWeeks=Object.fromEntries(PLATS.map(platform=>{const cutoff=D.coverage[platform],completeThrough=D.completeThrough[platform],weeks=D.categoryWeeks.filter(week=>week[1]<=cutoff).slice(-8).map(([key,start])=>{const scheduledEnd=shiftDate(start,6),end=scheduledEnd<cutoff?scheduledEnd:cutoff,posts=D.posts.filter(post=>post.p===platform&&post.cw===key&&post.d>=start&&post.d<=end),views=posts.reduce((sum,post)=>sum+post.v,0),eng=posts.reduce((sum,post)=>sum+post.e,0),complete=scheduledEnd<=completeThrough,partialReason=complete?null:(completeThrough<cutoff?"source-mix":"date-cutoff");return {key,start,end,complete,partialReason,views,eng,posts:posts.length,er:views?(100*eng/views).toFixed(2):null}});if(weeks.length!==8)throw new Error(`${platform} mini chart cannot show eight latest weeks`);return [platform,weeks]}));
...
const defaultSparkWeek=Object.fromEntries(PLATS.map(platform=>{const weeks=platformSparkWeeks[platform],latestComplete=weeks.slice().reverse().find(week=>week.complete);return [platform,(latestComplete||weeks[weeks.length-1]).key]}));
const sparkSelection={...defaultSparkWeek};
const sparkSummary=(platform,week)=>{const row=sparkWeekMap[platform][week],unit=D.definitions[platform].metric_label;return {value:`${fmt(row.views)} <span class="value-unit">${esc(unit)}</span>`,detail:`${chartDate(row.start)}–${chartDate(row.end)} · ${fmt(row.eng)} engagements · ${postCount(row.posts)} · ${row.er??"—"}% rate`}};
const partialCoverage=(platform,row)=>row.partialReason==="source-mix"?(coveredDays(row.start,row.end)<7?`Partial week + source mix · ${coveredDays(row.start,row.end)} of 7 days · long-form snapshot through ${chartDate(row.end)} · full Shorts catalog through ${chartDate(D.completeThrough[platform])}`:`Partial source mix · long-form snapshot through ${chartDate(row.end)} · full Shorts catalog through ${chartDate(D.completeThrough[platform])}`):`Partial publish week through ${chartDate(row.end)} · ${coveredDays(row.start,row.end)} of 7 days`;
const sparkDetail=(platform,week)=>{const row=sparkWeekMap[platform][week],unit=D.definitions[platform].metric_label,coverage=row.complete?(row.key===defaultSparkWeek[platform]?"Latest complete platform week":"Complete publish week"):partialCoverage(platform,row);return `<strong>Selected week · ${esc(chartDate(row.start))}–${esc(chartDate(row.end))}, ${row.start.slice(0,4)}</strong><span>${full(row.views)} ${unit} · ${full(row.eng)} engagements · ${postCount(row.posts)} · ${row.er??"—"}% rate · ${coverage}</span>`};
const spark=(platform,color)=>{const unit=D.definitions[platform].metric_label,selected=sparkSelection[platform],weeks=platformSparkWeeks[platform],max=Math.max(...weeks.map(week=>week.views),1),first=weeks[0],last=weeks[weeks.length-1];return `<svg class="spark" viewBox="0 0 160 38" preserveAspectRatio="none" role="group" aria-label="${esc(platform)} latest ${weeks.length} available publish-week ${esc(unit)} trend. Select a bar for exact weekly stats; outlined bars have partial source coverage.">${weeks.map((week,i)=>{const value=week.views,h=Math.max(1,32*value/max),isSelected=week.key===selected,coverage=week.complete?"complete publish week":partialCoverage(platform,week),label=`${platform}, ${chartDate(week.start)}–${chartDate(week.end)}, ${week.start.slice(0,4)}: ${full(value)} ${unit}, ${full(week.eng)} engagements, ${postCount(week.posts)}, ${week.er??"—"}% engagement rate, ${coverage}`;return `<g class="spark-bar ${isSelected?"selected":""}" role="button" tabindex="0" focusable="true" aria-pressed="${isSelected}" aria-label="${esc(label)}" data-spark-platform="${esc(platform)}" data-spark-week="${esc(week.key)}"><rect class="spark-hit" x="${i*20}" y="0" width="20" height="38"/><rect class="spark-fill${week.complete?"":" partial"}" x="${i*20+2}" y="${36-h}" width="14" height="${h}" rx="2" fill="${color}" opacity=".48"/><title>${esc(label)}</title></g>`}).join("")}</svg><div class="spark-range" aria-hidden="true"><span>${esc(chartDate(first.start))}</span><span>${esc(chartDate(last.start))}–${esc(chartDate(last.end))}${last.complete?"":last.partialReason==="source-mix"?` · <b>${coveredDays(last.start,last.end)<7?`partial ${coveredDays(last.start,last.end)}/7 + source mix`:"partial source mix"}</b>`:` · <b>partial ${coveredDays(last.start,last.end)}/7 days</b>`}</span></div>`};
...
const subCompleteThrough=new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",year:"numeric",timeZone:"UTC"}).format(new Date(SUB.complete_through+"T00:00:00Z"));
document.getElementById("subscribers").innerHTML=`<div><div class="label">2026 exported videos</div><div class="value">${full(SUB.gained)}</div><div class="detail">${esc(SUB.as_of_label||("Cumulative through "+subCutoff))}</div></div>
...
function renderSubscriberWeeks(){
 ...
 rows.map(row=>`<tr><th scope="row">${esc(subscriberDate(row.start))}–${esc(subscriberDate(row.end))}, ${row.end.slice(0,4)}</th><td class="n">${full(row.long_form)}</td><td class="n">${full(row.shorts)}</td><td class="n"><strong>${full(row.gained)}</strong></td><td class="n">${full(row.video_count)}</td><td>${!row.complete?(row.coverage_reason==="source-mix"?(row.covered_days<7?"Partial week + source mix — "+row.covered_days+" of 7 days; full Shorts through "+esc(subCompleteThrough):"Partial source mix — full Shorts through "+esc(subCompleteThrough)):"Partial week — cutoff "+esc(subCutoff)):row.video_count?"Complete publish week":"No exported videos"}</td></tr>`).join("")+
...
document.getElementById("platforms").innerHTML=PLATS.slice().sort((a,b)=>RS.totals[b].views-RS.totals[a].views).map(p=>{
 ...
 <div class="spark-help">Latest 8 available publish weeks. The latest complete week is selected; select any bar for exact stats. Outlined bars have partial source coverage.</div>${spark(p,PC[p])}<div class="spark-readout" aria-live="polite">${sparkDetail(p,sparkSelection[p])}</div><div class="detail">YTD: ${fmt(D.totals[p].views)} ${unit} across ${full(D.totals[p].posts)} posts</div></div>`}).join("");
...
const topCoverageText=(week,platform)=>{const row=D.top5Coverage[week][platform],weekStart=D.top5Weeks.find(item=>item[0]===week)[1];if(row.status==="complete")return `Complete week · ${postCount(row.posts)}`;if(row.status==="partial"&&row.partial_reason==="source-mix")return row.covered_days<7?`Partial week + source mix · ${chartDate(weekStart)}–${chartDate(row.coverage_end)} · ${row.covered_days} of 7 days · full Shorts catalog through ${chartDate(row.complete_through)} · ${postCount(row.posts)}`:`Partial source mix · long-form snapshot ${chartDate(weekStart)}–${chartDate(row.coverage_end)} · full Shorts catalog through ${chartDate(row.complete_through)} · ${postCount(row.posts)}`;if(row.status==="partial")return `Partial coverage · ${chartDate(weekStart)}–${chartDate(row.coverage_end)} · ${row.covered_days} of 7 days · ${postCount(row.posts)}`;return `Source unavailable after ${chartDate(row.source_end)} — not zero posts`};
